
document.querySelector("#menu").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "block";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    document.querySelector("#btn").style.display="block";
    

});
document.querySelector("#vcardplus").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "block";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#business").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "block";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#website").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "block";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#apps").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "block";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#wifi").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "block";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#video").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "block";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#pdf").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "block";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#images").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "block";
    document.querySelector(".listoflink").style.display = "none";
    

});
document.querySelector("#listoflinks").addEventListener("click",function(){
    document.querySelector(".menu").style.display = "none";
    document.querySelector(".vcardplus").style.display = "none";
    document.querySelector(".business").style.display = "none";
    document.querySelector(".website").style.display = "none";
    document.querySelector(".apps").style.display = "none";
    document.querySelector(".wifi").style.display = "none";
    document.querySelector(".video").style.display = "none";
    document.querySelector(".pdf").style.display = "none";
    document.querySelector(".images").style.display = "none";
    document.querySelector(".listoflinks").style.display = "block";
    

});


// document.querySelector("#vcardplus").addEventListener("click",function(){
//     document.querySelector(".vcardplus").style.display = "block";
//     document.querySelector(".mobile.content").style.display = "none";
//     console.log('vcard');

// });


